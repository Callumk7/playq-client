...
export const meta: MetaFunction = () => {
  return [
    { title: "playQ" },
    { name: "description", content: "What are you playing?" },
  ];
};
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const userPlaylistsPromise = await getCreatedAndFollowedPlaylists(
    session.user.id
  );
  ...
  const [userPlaylists, userFriends, friendActivity, userCollection] =
    await Promise.all([
...
export default function AppLayout() {
  ...
  const setUserCollection = useUserCacheStore(
    (state) => state.setUserCollection
  );
