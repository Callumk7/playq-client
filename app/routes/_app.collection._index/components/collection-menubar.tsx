...
export function CollectionMenubar({
	...
	const userPlaylists = useUserCacheStore((state) => state.userPlaylists);
	...
	const handleBulkDelete = () => {
		...
		deleteFetcher.submit(formData, { method: "DELETE", action: "/api/collections/bulk" });
	};
						...
						<DropdownMenuSub>
							...
							<DropdownMenuSubContent>
								{userPlaylists.map((playlist) => (
									<DropdownMenuItem
										key={playlist}
										onClick={() => handleBulkAddToPlaylist(playlist)}
									>
										{playlist}
									</DropdownMenuItem>
								))}
